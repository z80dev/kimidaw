/**
 * Tension AudioWorklet Processor
 */

import { TensionVoiceAllocator } from '../dsp/voice-allocator.js';
...
class TensionProcessor extends AudioWorkletProcessor {
  private voiceAllocator: TensionVoiceAllocator | null = null;
  private currentState: TensionState | null = null;
  ...
  private handleMessage(message: WorkletMessage): void {
    switch (message.type) {
      case 'state':
        const { state } = message.payload as { state: TensionState };
  ...
  private updateState(state: TensionState): void {
    ...
    if (voicesChanged || !this.voiceAllocator) {
      this.voiceAllocator = new TensionVoiceAllocator(
        state.global.voices,
        this.sampleRate,
        state
      );
...
registerProcessor('tension-instrument', TensionProcessor);
