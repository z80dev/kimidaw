 ...
 */
export function interpolateValue(t, // 0 to 1 position between points
startValue, endValue, mode, curveIn, curveOut) {
    switch (mode) {
        case 'step':
            return startValue;
        case 'linear':
        ...
        case 'bezier':
            return interpolateBezier(t, startValue, endValue, curveIn ?? 0, curveOut ?? 0);
        default:
            return startValue + (endValue - startValue) * t;
    }
}
 ...
 */
function interpolateBezier(t, p0, p3, curveIn, curveOut) {
 ...
 */
export function getValueAtTick(lane, tick) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        if (tick >= p1.tick && tick <= p2.tick) {
            ...
            return interpolateValue(t, p1.value, p2.value, lane.interpolation, p1.curveOut, p2.curveIn);
        }
    }
    return undefined;
}
 ...
 */
export function addPoint(lane, tick, value, options) {
 ...
 */
export function removePoint(lane, pointIndex) {
    return {
        ...lane,
        points: [
 ...
 */
export function removePointsInRange(lane, startTick, endTick) {
    return {
        ...lane,
        points: lane.points.filter((p) => p.tick < startTick || p.tick > endTick),
    };
}
/**
 * Scale automation values
 */
export function scaleValues(lane, factor) {
    return {
        ...lane,
 ...
 */
export function shiftInTime(lane, offsetTicks) {
    return {
        ...lane,
        points: lane.points.map((p) => ({
            ...p,
 ...
 */
export function automationTargetKey(target) {
    return `${target.scope}:${target.ownerId}:${target.paramId}`;
}
/**
 * Parse an automation target key
 */
export function parseAutomationTargetKey(key) {
 ...
 */
export function createAutomationLane(id, target, options) {
    return {
        id,
        target,
 ...
 */
export function createTrackAutomationLane(id, trackId, paramId, options) {
    return createAutomationLane(id, { scope: 'track', ownerId: trackId, paramId }, options);
}
/**
 * Create an automation lane for a plugin parameter
 */
export function createPluginAutomationLane(id, pluginId, paramId, options) {
    return createAutomationLane(id, { scope: 'plugin', ownerId: pluginId, paramId }, options);
}
// ==================== Preset Values ====================
 ...
 */
export function normalizeValue(targetId, value) {
 ...
 */
export function denormalizeValue(targetId, normalized) {
    const range = AUTOMATION_RANGES[targetId];
    if (!range) {
        return normalized;
    }
